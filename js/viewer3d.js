...
(function () {
  ...
  function paintLabel(sprite) {
    ...
    g.fillStyle = "rgba(251,250,246,0.92)";       /* paper pill on the dusk sky */
    g.fill();
    g.strokeStyle = "rgba(22,19,16,0.30)";
    g.lineWidth = 1;
    g.stroke();
    g.beginPath();
  ...
  window.initViewer3D = function (cfg) {
    ...
    try {
      renderer.setClearColor(0x2b2530, 1);
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));
      // filmic dusk: sRGB output + ACES, soft shadows
      ...
      if (matBrick.envMapIntensity !== undefined) { matBrick.envMapIntensity = 1.1; matClip.envMapIntensity = 0.7; }
      ...
      var HOME = { r: diag * 1.14, theta: -Math.PI / 2 - 0.62, phi: 1.38 }; // low, near eye level (contre-jour shot)

      // ----- COLD CLEAR evening sky (contre-jour): pale luminous horizon -----
      ...
      grad.addColorStop(0.00, "#202b3a");   // cold zenith (matches CSS on #viewer3d)
      grad.addColorStop(0.30, "#33465c");
      grad.addColorStop(0.44, "#6e8ba4");
      grad.addColorStop(0.50, "#d9e8f2");   // luminous pale band right at the horizon
      grad.addColorStop(0.56, "#93a6b5");
      grad.addColorStop(1.00, "#55616c");   // below horizon
      ...
      scene.background = bgTex;   // fallback behind the dome

      // ----- image-based lighting: dusk equirect -> PMREM env -----
      // Warm amber ambient wraps the brick; the steel clips pick up the sky.
      try {
      ...
      var fogCol = new THREE.Color(0x93a6b5);          // cool haze toward the horizon
      ...
      var fill = new THREE.DirectionalLight(0xd8dcdf, 0.34);          // neutral fill from the camera side (faces stay readable, not blue)
      fill.position.set(center.x - diag * 0.6, center.y - diag * 0.7, center.z + diag * 0.5);
      fill.target.position.copy(center);
      scene.add(fill); scene.add(fill.target);

      // sky dome: the gradient tracks the TRUE horizon at every camera angle
      // (equator = amber band sits exactly at ground level)
      var groundZ0 = bb.min[2] - 0.01;
      (function () {
        var skyGeo = new THREE.SphereGeometry(diag * 7, 32, 24);
        var skyMat = new THREE.MeshBasicMaterial({ map: bgTex, side: THREE.BackSide, fog: false });
        if (skyMat.toneMapped !== undefined) skyMat.toneMapped = false;
        var sky = new THREE.Mesh(skyGeo, skyMat);
        sky.rotation.x = Math.PI / 2;            // poles onto the z-up axis
        sky.position.set(center.x, center.y, groundZ0);
        sky.renderOrder = -2;
        scene.add(sky);
      })();

      // visible warm earth ground; receives the long dusk shadows, fades into the fog
      var groundZ = groundZ0;
      var groundCol = new THREE.Color(0x6e7880);
      if (groundCol.convertSRGBToLinear) groundCol.convertSRGBToLinear();
      var ground = new THREE.Mesh(
        new THREE.CircleGeometry(diag * 6.0, 64),
        new THREE.MeshStandardMaterial({ color: groundCol, roughness: 1.0, metalness: 0.0 })
      );
      ground.position.set(center.x, center.y, groundZ);
      ground.receiveShadow = true;
      scene.add(ground);

      // low sun glow billboard on the key-light side, sitting on the horizon
      (function () {
        var sc = document.createElement("canvas");
        sc.width = sc.height = 256;
        var sg = sc.getContext("2d");
        var srg = sg.createRadialGradient(128, 128, 6, 128, 128, 128);
        srg.addColorStop(0.00, "rgba(255,248,232,0.98)");
        srg.addColorStop(0.22, "rgba(255,238,206,0.6)");
        srg.addColorStop(0.60, "rgba(224,235,244,0.2)");
        srg.addColorStop(1.00, "rgba(224,235,244,0)");
        sg.fillStyle = srg; sg.fillRect(0, 0, 256, 256);
        var sunTex = new THREE.CanvasTexture(sc);
        ...
        var kd = new THREE.Vector3().copy(key.position).sub(center).normalize();
        sun.position.set(center.x + kd.x * diag * 3.0, center.y + kd.y * diag * 3.0, groundZ + diag * 0.26);
        sun.scale.set(diag * 1.5, diag * 1.5, 1);
        sun.renderOrder = -1;
        scene.add(sun);
      })();
      ...
      pool.position.set(center.x, center.y, groundZ + 0.02);  // sits ON the visible ground
      scene.add(pool);
      ...
      return {
        ...
        viewAll: function () { controls.set(center, diag * 1.35, HOME.theta, 1.3); },
        resize: resize
      };
    } catch (e) {
      fail();
