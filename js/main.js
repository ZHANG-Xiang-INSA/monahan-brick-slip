...
(function () {
  ...
  function dvEntries() {
    ...
    out.push({ code: "CUT-02", name: T("cut_clips"), sub: T("cut_clips_sub"), src: "svg/cut_clip_types.svg?b=34", key: "cut2", w: 2000, h: 2218 });
    return out;
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // The toolbar now lives INSIDE #viewer3d; OrbitLite pointer-captures the
    // container on pointerdown, which would retarget clicks away from the
    // buttons. Stop interaction events from reaching the orbit controls.
    var vtb = container.querySelector(".stage-toolbar");
    if (vtb) {
      ["pointerdown", "mousedown", "touchstart", "wheel", "dblclick"].forEach(function (ev) {
        vtb.addEventListener(ev, function (e) { e.stopPropagation(); });
      });
    }
